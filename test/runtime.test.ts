...
//         expect((await runQuery(query)).Todo).to.have.members([1, 2, 3]);
//         expect((await runQuery(query)).Next).to.equal(1);
//     });

//     it("Querying concurrently works", async () => {
//         createSession(logic);
//         return Promise.all([
//             runQuery("visible(Todo).")
//                 .then(x => expect(x.Todo).to.have.members([1, 2, 3])),
//             runQuery("next_todo = Next.")
...
//     it("Dispatch", async () => {
//         createSession(logic);
//         await dispatch("set_active_filter", { filter: "completed" });
//         const result1 = await runQuery("active_filter = Active.");
//         expect(result1.Active).to.equal("completed");
//         const result2 = await runQuery("visible(Todo).");
